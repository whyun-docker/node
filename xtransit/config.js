...
const notImportantOptionalConfig = {
  ...
  libMode: ENV.EZM_LIB_MODE === 'true' || false, // 默认 false，此时采集如果收到 shutdown 事件会退出当前进程；如果是以第三方库的形式引用接入应用内，请将此属性设置为 true
  ...
  customAgent: ENV.EZM_CUSTOM_AGENT ||  undefined, // 默认为 undefined，如果配置为一个函数，则可以通过此函数自定义 agent 标识
}

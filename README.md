...
## 版本
> `x.y.z` 版本号在推送的时候，会级联推送 `x.y` 和 `x` 版本。 应用会自动抓取 node 官方网站中各个 LTS 版本，并级联推送。目前支持 18+ LTS 版本，低于 18 版本的不会推送。

## GitHub Actions 配置

### 推送至阿里云 ACR

本项目已支持自动推送 Docker 镜像到阿里云容器镜像服务 (ACR)。需要在 GitHub Repository Settings 中配置以下 Secrets 和 Variables：

#### 必需的 Secrets：
- `DOCKERHUB_USERNAME` - Docker Hub 用户名
- `DOCKERHUB_TOKEN` - Docker Hub Access Token
- `ACR_USERNAME` - 阿里云 ACR 用户名（AccessKey ID 或 RAM 用户名）
- `ACR_PASSWORD` - 阿里云 ACR 密码（AccessKey Secret 或 RAM 用户密码）

#### 可选的 Variables（在 Repository Settings > Secrets and variables > Actions > Variables 中配置）：
- `ACR_REGISTRY` - ACR 地域地址，默认为 `registry.cn-hangzhou.aliyuncs.com`
...
- `ACR_NAMESPACE` - ACR 命名空间（必须配置才会推送至 ACR）

#### 推送的镜像地址格式：
- `registry.cn-hangzhou.aliyuncs.com/{namespace}:node-{tag}`
- `registry.cn-hangzhou.aliyuncs.com/{namespace}:node-compiler-{tag}`
- `registry.cn-hangzhou.aliyuncs.com/{namespace}:node-xtransit-{tag}`
- `registry.cn-hangzhou.aliyuncs.com/{namespace}:easy-monitor-{tag}`

当 `ACR_NAMESPACE` 变量未配置或为空时，不会执行 ACR 推送操作。
